...
public class TERDecal extends TreeElementRenderable<ElementDecal> {
	...
	public TERDecal(ElementDecal object) {
		...
		for (int i = 0; i != 6; i++) {
			Vector3 vector = i < 3 ? renderable.getTransform(GizmoTransformType.TRANSLATE) : renderable.getTransform(GizmoTransformType.ROTATE);
			...
			protected int renderInside(int y, Batch batch, ShapeDrawer shape, Foster foster) {
				for (int i = 0; i != 2; i++) {
					...
					for (int j = 0; j != 3; j++) {
						...
						drawWheelInput(TERDecal.this.renderable.getTransform(GizmoTransformType.values()[i]), j, shape, this.x + 24, y - 20, 12, 14);
