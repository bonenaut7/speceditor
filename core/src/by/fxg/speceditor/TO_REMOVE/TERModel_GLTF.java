...
/** Currently not supported type of object. 
 *  GLTF Models can be loaded as default objects with gdx-gltf loader.
**/
@Deprecated
public class TERModel_GLTF extends TreeElementRenderable<ElementModel> {
	...
	public TERModel_GLTF(ElementModel object) {
		super(object);
		...
		this.attributeSelection = new UDropdownSelectSingle(0, 0, 0, 0, 15, "None", "[TEX] Diffuse", "[TEX] Emissive");
		...
		this.blocks[1] = new URenderBlock("Transform", 0, 0, 0) {
			protected int renderInside(int y, Batch batch, ShapeDrawer shape, Foster foster) {
				TERModel_GLTF ter = TERModel_GLTF.this;
				for (int i = 0; i != 3; i++) {
					switch(i) {
					...
					for (int j = 0; j != 3; j++) {
						...
						drawWheelInput(ter.renderable.getTransform(GizmoTransformType.values()[i]), j, shape, this.x + 24, y - 20, 12, 14);
						...
						ter.input[j + i * 3].setTransforms(this.x + 40, y -= 20, this.width - 40, 15).update();
						ter.input[j + i * 3].render(batch, shape, foster);
					}
				}
				y += 8;
				return y;
							...
							if (!attributeSelection.isDropped()) {
								...
								if (attributeSelection.getVariant() < 4) { //tex attribs
									...
									switch (attributeSelection.getVariant()) {
										case 1: attributeID = PBRTextureAttribute.BaseColorTexture; break;
										case 2: attributeID = PBRTextureAttribute.EmissiveTexture; break;
									}
									Attribute rawAttribute = renderable.modelInstance.materials.get(materialSelection.getVariant() - 1).get(attributeID);
									if (rawAttribute instanceof TextureAttribute && !(rawAttribute instanceof PBRTextureLinkedAttribute)) { //check for invalid attribute for recreation
									...
									PBRTextureLinkedAttribute attribute = (PBRTextureLinkedAttribute)renderable.modelInstance.materials.get(materialSelection.getVariant() - 1).get(attributeID);
									if (attribute == null) {
										...
										if (attributeButtons[0].isPressed()) {
											try {
												...
												attribute = new PBRTextureLinkedAttribute(Long.valueOf(attributeID), new Texture(handle), handle);
										...
										if (attributeButtons[2].isPressed()) {
											...
											renderable.modelInstance.materials.get(materialSelection.getVariant() - 1).set(new PBRTextureLinkedAttribute(Long.valueOf(attributeID), textureRegion, attribute.texturePath).setFlip(false, false));
										} else if (attributeButtons[3].isPressed()) {
											TextureRegion textureRegion = new TextureRegion(attribute.textureDescription.texture);
											textureRegion.flip(false, true);
											renderable.modelInstance.materials.get(materialSelection.getVariant() - 1).set(new PBRTextureLinkedAttribute(Long.valueOf(attributeID), textureRegion, attribute.texturePath).setFlip(false, true));
										} else if (attributeButtons[4].isPressed()) {
											TextureRegion textureRegion = new TextureRegion(attribute.textureDescription.texture);
											textureRegion.flip(true, false);
											renderable.modelInstance.materials.get(materialSelection.getVariant() - 1).set(new PBRTextureLinkedAttribute(Long.valueOf(attributeID), textureRegion, attribute.texturePath).setFlip(true, false));
										} else if (attributeButtons[5].isPressed()) {
											TextureRegion textureRegion = new TextureRegion(attribute.textureDescription.texture);
											textureRegion.flip(true, true);
											renderable.modelInstance.materials.get(materialSelection.getVariant() - 1).set(new PBRTextureLinkedAttribute(Long.valueOf(attributeID), textureRegion, attribute.texturePath).setFlip(true, true));
										}
	...
	public void update(int x, int y, int width, int height, boolean allowMouse) {
		...
		if (this.modelSelectionButton.isPressed()) {
			try {
				...
				this.updateModel(handle, Game.get.resourceManager.assetManager.get(assetDescriptor));
			} catch (Exception e) {}
		}
	}
