...
public class UDropdownSelectSingle extends UIElement implements IFocusable {
	...
	public UDropdownSelectSingle(int x, int y, int width, int height, int dropHeight, String... variants) { this(0, x, y, width, height, dropHeight, variants); }
	public UDropdownSelectSingle(int selectedVariant, int x, int y, int width, int height, int dropHeight, String... variants) {
		...
		if (this.isFocused()) {
			...
			if (this.isMouseOver()) {
				...
				if (this.getInput().isMouseScrolled(true)) {
					...
					if (this.selectedVariant >= this.variants.length) this.selectedVariant = 0;
				} else if (this.getInput().isMouseScrolled(false)) {
					this.selectedVariant--;
					if (this.selectedVariant < 0) this.selectedVariant = this.variants.length - 1;
				}
			}
		}
	}
	...
	public void render(ShapeDrawer shape, Foster foster) {
		if (this.isDropped()) {
			...
			for (int i = 0; i != this.variants.length; i++) {
				...
				if (GDXUtil.isMouseInArea(this.x, this.y - this.dropHeight * i - this.dropHeight - 2, this.width, this.dropHeight)) {
