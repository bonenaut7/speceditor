...
import static by.fxg.pilesos.specformat.graph.SpecPointArray.Flags.MODE_LINE;
import static by.fxg.pilesos.specformat.graph.SpecPointArray.Flags.MODE_LOOP;
import static by.fxg.pilesos.specformat.graph.SpecPointArray.Flags.hasFlag;
import static by.fxg.pilesos.specformat.graph.SpecPointArray.Flags.invertFlag;
...
public class TERPointArray extends TreeElementRenderable<ElementPointArray> {
	...
	public TERPointArray(ElementPointArray object) {
		super(object);
		
		String numeral = "0123456789-.";
		this.nameInput = new UInputField(0, 0, 0, 0).setMaxLength(24).setText(renderable.getName());
		for (int i = 0; i != 3; i++) {
			this.posInput[i] = new UInputField(0, 0, 0, 0).setAllowedCharacters(numeral).setMaxLength(10).setText(String.valueOf(i == 0 ? renderable.getTransform(EnumTransform.TRANSLATE).x : i == 1 ? renderable.getTransform(EnumTransform.TRANSLATE).y : renderable.getTransform(EnumTransform.TRANSLATE).z));
		}
		...
		this.blocks[1] = new URenderBlock("Transform", 0, 0, 0) {
			protected int renderInside(int y, Batch batch, ShapeDrawer shape, Foster foster) {
				...
				for (int i = 0; i != 3; i++) {
					...
					drawWheelInput(renderable.getTransform(EnumTransform.TRANSLATE), i, shape, this.x + 24, y - 20, 12, 14);
		...
		this.blocks[2] = new URenderBlock("Flags", 0, 0, 0) {
			protected int renderInside(int y, Batch batch, ShapeDrawer shape, Foster foster) {
				y -= 5;
				String[] names = {"Loop points", "Line points"};
				long[] flags = {MODE_LOOP, MODE_LINE};
	...
	public void resetInputFields() {
		...
		Vector3 position = this.renderable.getTransform(EnumTransform.TRANSLATE);
		for (int i = 0; i != 3; i++) {
