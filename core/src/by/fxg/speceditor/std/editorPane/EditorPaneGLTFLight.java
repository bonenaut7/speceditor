...
public class EditorPaneGLTFLight extends EditorPane implements ISTDInputFieldListener {
	private ElementGLTFLight element = null;
	private UDropdownSelectSingle lightType;
	private STDInputField elementName;
	private TransformBlock transform;
	
	public EditorPaneGLTFLight() {
						...
						case 0: { 
							switch(element.type) {
								case SPOT: {
									...
									pointLight.setColor(element.getLight(SpotLightEx.class).color);
									pointLight.setPosition(element.getLight(SpotLightEx.class).position);
									pointLight.intensity = element.getLight(SpotLightEx.class).intensity;
									pointLight.range = element.getLight(SpotLightEx.class).range;
						...
						case 1: { 
							switch(element.type) {
								case POINT: {
									...
									SpotLightEx spotLight = new SpotLightEx();
									spotLight.setColor(element.getLight(PointLightEx.class).color);
									spotLight.setPosition(element.getLight(PointLightEx.class).position);
									spotLight.intensity = element.getLight(PointLightEx.class).intensity;
									spotLight.range = element.getLight(PointLightEx.class).range;
	...
	public void updatePane(ITreeElementSelector<?> selector) {
		this.element = (ElementGLTFLight)selector.get(0);
	...
	public boolean acceptElement(ITreeElementSelector<?> selector) {
		return selector.size() == 1 && selector.get(0) instanceof ElementGLTFLight;
	}
	...
	private class TransformBlock extends URenderBlock implements ISTDInputFieldListener {
		...
		private EditorPaneGLTFLight parent;
		...
		private TransformBlock(EditorPaneGLTFLight parent) {
			super("Transforms");
			this.parent = parent;
		...
		public void whileInputFieldFocused(STDInputField inputField, String id) {
			switch (id) {
				...
				case "intensity": {
					if (this.lightType == ElementLightType.POINT) ((PointLightEx)this.light).intensity = UIElement._convertTextToFloat(this.parameters[0], ((PointLightEx)this.light).intensity);
					else if (this.lightType == ElementLightType.SPOT) ((SpotLightEx)this.light).intensity = UIElement._convertTextToFloat(this.parameters[0], ((SpotLightEx)this.light).intensity);
				} break;
				case "cutoffangle": ((SpotLightEx)this.light).cutoffAngle = UIElement._convertTextToFloat(this.parameters[1], ((SpotLightEx)this.light).cutoffAngle); break;
				case "exponent": ((SpotLightEx)this.light).exponent = UIElement._convertTextToFloat(this.parameters[2], ((SpotLightEx)this.light).exponent); break;
			}
		}
		...
		private void updateBlock(ElementGLTFLight light) {
			...
			if (this.lightType == ElementLightType.POINT) {
				this.parameters[0].setTextWithPointer(String.valueOf(((PointLightEx)this.light).intensity)).dropOffset();
			} else if (this.lightType == ElementLightType.SPOT) {
				SpotLightEx spotLight = (SpotLightEx)this.light;
