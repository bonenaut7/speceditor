...
public class ScreenCreateProject extends BaseScreen implements ISTDInputFieldListener {
	...
	public void update(Batch batch, ShapeDrawer shape, Foster foster, int width, int height) {
		...
		if (this.buttonReturn.isPressed()) {
			SpecEditor.get.renderer.currentScreen = this.parent;
		}
		...
		if (SpecEditor.get.getInput().isMouseDown(0, false) && GDXUtil.isMouseInArea(5, 22, width / 5 - 12, height - 60)) {
			this.selectedSolverIndex = (height - 40 - GDXUtil.getMouseY() + (int)this.scroll) / 20;
			if (this.displaySolvers.size <= this.selectedSolverIndex) {
				this.selectedSolverIndex = -1;
				this.selectedSolverSubscreen = null;
			} else {
				this.selectedSolverSubscreen = this.displaySolvers.get(this.selectedSolverIndex).getProjectCreationSubscreen();
				this.selectedSolverSubscreen.resize(width / 5 + 1, 2, width - width / 5 - 3, height - 5);
			}
		}
	...
	public void resize(int width, int height) {
		Foster foster = SpecEditor.fosterNoDraw;
