...
public class ScreenProject extends BaseScreen {
	...
	//public __$$Project project;
	
	public ScreenProject() {//__$$Project project) {
		...
		this.subProjectManager = new SubscreenProjectManager();
		this.subExplorer = new SubscreenExplorer();
		...
		//project.loadProject(this.subProjectManager.objectTree);
	}
	...
	public void update(Batch batch, ShapeDrawer shape, Foster foster, int width, int height) {
		...
		if (this.udcProject.isPressed()) {
				...
				case 0: {
					...
					//this.project.saveProject(this.subProjectManager.objectTree);
				} break;
				case 1: {
					...
					fileChooser.setCurrentDirectory(ProjectManager.currentProject.getProjectFolder().parent().parent().file());
					if (fileChooser.showSaveDialog(frame) == JFileChooser.APPROVE_OPTION && fileChooser.getSelectedFile() != null) {
						...
						//exporter.export(this.project, inputFolder.child("scene.sfs"), SpecFormatConverter.convertToGraph(this.project, this.subProjectManager.objectTree, true));
					}
				} break;
				case 2: {
					try { Desktop.getDesktop().open(ProjectManager.currentProject.getProjectFolder().file()); } catch (Exception e) {}
				} break;
			}
		}
	}
