...
public class SmartFontGenerator {
	...
	public BitmapFont createFont(String chars, FileHandle fontFile, String fontName, int fontSize) {
		BitmapFont font = null; Preferences fontPrefs = Gdx.app.getPreferences("org.jrenner.smartfont"); int displayWidth = fontPrefs.getInteger("display-width", 0); int displayHeight = fontPrefs.getInteger("display-height", 0); boolean loaded = false;
		...
		if (!loaded || forceGeneration) {
			forceGeneration = false; fontPrefs.putInteger("display-width", Gdx.graphics.getWidth()); fontPrefs.putInteger("display-height", Gdx.graphics.getHeight()); fontPrefs.flush();
			font = generateFontWriteFiles(chars, fontName, fontFile, fontSize, pageSize, pageSize);
		}
		return font;
	}
	
	private BitmapFont generateFontWriteFiles(String chars, String fontName, FileHandle fontFile, int fontSize, int pageWidth, int pageHeight) {
		FreeTypeFontGenerator generator = new FreeTypeFontGenerator(fontFile); PixmapPacker packer = new PixmapPacker(pageWidth, pageHeight, Pixmap.Format.RGBA8888, 2, false);
		FreeTypeFontParameter parameter = new FreeTypeFontParameter(); parameter.size = fontSize; parameter.characters = chars; parameter.flip = false; parameter.packer = packer;
		FreeTypeFontGenerator.FreeTypeBitmapFontData fontData = generator.generateData(parameter);
		Array<PixmapPacker.Page> pages = packer.getPages(); Array<TextureRegion> texRegions = new Array<>();
		for (int i = 0; i < pages.size; i++) { PixmapPacker.Page p = pages.get(i);
			Texture tex = new Texture( new PixmapTextureData(p.getPixmap(), p.getPixmap().getFormat(), false, false, true)) { public void dispose() { super.dispose(); getTextureData().consumePixmap().dispose(); } };
			tex.setFilter(Texture.TextureFilter.Nearest, Texture.TextureFilter.Nearest); texRegions.add(new TextureRegion(tex));
		}
		BitmapFont font = new BitmapFont((BitmapFont.BitmapFontData) fontData, texRegions, false); generator.dispose(); packer.dispose();
		return font;
	}
...
//			}
	...
	private BitmapFont generateFontWriteFiles(String fontName, FileHandle fontFile, int fontSize, int pageWidth, int pageHeight) {
		...
		BitmapFont font = new BitmapFont((BitmapFont.BitmapFontData) fontData, texRegions, false);
			//saveFontToFile(font, fontSize, fontName, packer); <-- Deleted
		generator.dispose();
		packer.dispose();
		return font;
	}
